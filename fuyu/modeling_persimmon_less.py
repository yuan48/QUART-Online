...
# from transformers.models.persimmon.configuration_persimmon import FuyuConfig_less

from models.fuyu.Fuyu_Config import FuyuConfig_less


logger = logging.get_logger(__name__)

_CONFIG_FOR_DOC = "FuyuConfig_less"
...
class PersimmonAttention(nn.Module):
    ...
    def __init__(self, config: FuyuConfig_less, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class PersimmonDecoderLayer(nn.Module):
    def __init__(self, config: FuyuConfig_less, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
...
class PersimmonModel(PersimmonPreTrainedModel):
    ...
    def __init__(self, config: FuyuConfig_less):
    ...
    def forward(
        ...
        # for decoder_layer in self.layers:
            ...
            # if output_hidden_states:
                ...
                # print("all_hidden_states adding len:{},size: {}; hidden_states: {}".format(len(all_hidden_states),all_hidden_states[0].size(),hidden_states.size()))
            ...
            # print("decoder_layer:{}".format(decoder_layer))
        ...
        # print('hidden_states before final_layernorm size:', hidden_states.size())
        hidden_states = self.final_layernorm(hidden_states)
        # print('hidden_states after final_layernorm size:', hidden_states.size())

        # add hidden states from the last decoder layer
        # if output_hidden_states:
        if True:
            all_hidden_states += (hidden_states,)
            # print("all_hidden_states:{}, hidden_states: {}".format(all_hidden_states,hidden_states))
            # print("states len:{}, all states size:{}, hidden_states size:{}".format(len(all_hidden_states),all_hidden_states[0].size(),hidden_states.size()))

        # print("=============")

        # hidden_states=all_hidden_states[24]  #截断到12
...
class PersimmonForCausalLM_Less(PersimmonPreTrainedModel):
