...
class RVQ(torch.nn.Module):
    def __init__(
        ...
        # self.type="conv"   #choose conv, mlp
        ...
        if NetType == "mlp":
            ...
            self.encoder.add_module("encoder_input", nn.Conv1d(1, layers_hidden[0], kernel_size=3, stride=1, padding=1))
            for i in range(len(layers_hidden) - 1):
                self.encoder.add_module(f"encoder_act_{i}", nn.ReLU())
                self.encoder.add_module(f"encoder_hidden_{i}", nn.Conv1d(layers_hidden[i], layers_hidden[i + 1], kernel_size=3, stride=1, padding=1))
                
                # Define convolutional layers for decoder
                self.decoder.add_module(f"decoder_hidden_{i}", nn.Conv1d(layers_hidden[len(layers_hidden) - 1 - i], layers_hidden[len(layers_hidden) - 2 - i], kernel_size=3, stride=1, padding=1))
                self.decoder.add_module(f"decoder_act_{i}", nn.ReLU())
            self.decoder.add_module("decoder_output", nn.Conv1d(layers_hidden[0], 1, kernel_size=3, stride=1, padding=1))
    ...
    def loss_function(self,
        ...
        #sigmoid + 分类割出来，进四个loss，目标是0.001级别