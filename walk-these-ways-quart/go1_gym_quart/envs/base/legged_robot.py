...
from go1_gym_quart.envs.base.base_task import BaseTask
from go1_gym_quart.utils.math_utils import quat_apply_yaw, wrap_to_pi, get_scale_shift
...
class LeggedRobot(BaseTask):
    ...
    def step(self, actions):
        ...
        self.render_gui()
        ###wangzt
        self.imgs,self.depths = self.render()
        # img = cv2.cvtColor(self.imgs[0], cv2.COLOR_RGBA2BGR)
        # img = self.imgs[0]
        #WX:改，试图去掉可视化
        # cv2.imshow('fff',img)
        # cv2.waitKey(1)
        ...
        return self.obs_buf, self.privileged_obs_buf, self.rew_buf, self.reset_buf, self.extras, self.imgs, self.depths
    ...
    def post_physics_step(self):
        ...
        # prepare quantities
        self.base_pos[:] = self.root_states[self.ids_sim['robot'], 0:3]
        self.base_quat[:] = self.root_states[self.ids_sim['robot'], 3:7]

        ###wangzt
        # quat = gymapi.Quat()
        # quat.x,quat.y,quat.z,quat.w = self.base_quat[0].cpu().numpy()[:]
        # print(quat.to_euler_zyx())
        ####
        
        self.base_lin_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[self.ids_sim['robot'], 7:10])
        self.base_ang_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[self.ids_sim['robot'], 10:13])
        ...
        self.last_root_vel[:] = self.root_states[self.ids_sim['robot'], 7:13]
    ...
    def check_termination(self):
        ...
        if self.cfg.rewards.use_terminal_body_height:
            self.body_height_buf = torch.mean(self.root_states[self.ids_sim['robot'], 2].unsqueeze(1) - self.measured_heights, dim=1) \
    ...
    #     print('ddddddddddddddddddddd','set_idx_pose')
    #     if len(env_ids) == 0:
    #         return
    ...
    def compute_observations(self):
        ...
        if self.cfg.env.observe_vel:
            if self.cfg.commands.global_reference:
                self.obs_buf = torch.cat((self.root_states[self.ids_sim['robot'], 7:10] * self.obs_scales.lin_vel,
        ...
        # print("observe_command", self.cfg.env.observe_command)
        # print("observe_two_prev_actions", self.cfg.env.observe_two_prev_actions)
        # print("observe_timing_parameter", self.cfg.env.observe_timing_parameter)
        # print("observe_clock_inputs", self.cfg.env.observe_clock_inputs)
        # print("observe_vel", self.cfg.env.observe_vel)
        # print("observe_only_ang_vel", self.cfg.env.observe_only_ang_vel)
        # print("observe_only_lin_vel", self.cfg.env.observe_only_lin_vel)
        # print("observe_yaw", self.cfg.env.observe_yaw)
        # print("observe_contact_states", self.cfg.env.observe_contact_states)
        # print("add_noise", self.add_noise)
        # print(self.cfg.commands.num_commands)
        # print(self.cfg.rewards.kappa_gait_probs)
        # print(self.obs_scales)
        ...
        if self.cfg.env.priv_observe_body_height:
            ...
            self.privileged_obs_buf = torch.cat((self.privileged_obs_buf,
                                                 ((self.root_states[self.ids_sim['robot'], 2]).view(
    ...
    #     print('ffffffffffff','set_main_agent_pose')
    ...
    def _reset_dofs(self, env_ids, cfg):
        ...
        robot_indices = self.ids_sim["robot"][env_ids].to(torch.int32)
    ...
    def _reset_root_states(self, env_ids, cfg):
        ...
        #     self.root_states[self.ids_sim['robot'][env_ids]] = self.base_init_state
        #     self.root_states[self.ids_sim['robot'][env_ids], :3] += self.env_origins[env_ids]
        #     self.root_states[self.ids_sim['robot'][env_ids], 0:1] += torch_rand_float(-cfg.terrain.x_init_range,
        #                                                        cfg.terrain.x_init_range, (len(env_ids), 1),
        #                                                        device=self.device)
        #     self.root_states[self.ids_sim['robot'][env_ids], 1:2] += torch_rand_float(-cfg.terrain.y_init_range,
        #                                                        cfg.terrain.y_init_range, (len(env_ids), 1),
        #                                                        device=self.device)
        #     self.root_states[self.ids_sim['robot'][env_ids], 0] += cfg.terrain.x_init_offset
        #     self.root_states[self.ids_sim['robot'][env_ids], 1] += cfg.terrain.y_init_offset
        # else:
        #     self.root_states[self.ids_sim['robot'][env_ids]] = self.base_init_state
        #     self.root_states[self.ids_sim['robot'][env_ids], :3] += self.env_origins[env_ids]
        ...
        #                              device=self.device)
        # init_yaws = torch.zeros_like(init_yaws)
        # quat = quat_from_angle_axis(init_yaws, torch.Tensor([0, 0, 1]).to(self.device))[:, 0, :]
        # self.root_states[self.ids_sim['robot'][env_ids], 3:7] = quat

        # base velocities
        self.root_states[self.ids_sim['robot'][env_ids], 7:13] = torch_rand_float(-0.5, 0.5, (len(env_ids), 6),
        ...
        all_indices = torch.unique(torch.cat([indices[env_ids].to(torch.int32) for indices in self.ids_sim.values()]))
    ...
    def _push_robots(self, env_ids, cfg):
        ...
        if cfg.domain_rand.push_robots:
            ...
            self.root_states[self.ids_sim['robot'], 7:9] = torch_rand_float(-max_vel, max_vel, (len(env_ids), 2),
    ...
    def _teleport_robots(self, env_ids, cfg):
        ...
        if cfg.terrain.teleport_robots:
            ...
            low_x_ids = env_ids[self.root_states[self.ids_sim['robot'], 0] < thresh + x_offset]
            self.root_states[low_x_ids, 0] += cfg.terrain.terrain_length * (cfg.terrain.num_rows - 1)

            high_x_ids = env_ids[
                self.root_states[self.ids_sim['robot'], 0] > cfg.terrain.terrain_length * cfg.terrain.num_rows - thresh + x_offset]
            self.root_states[high_x_ids, 0] -= cfg.terrain.terrain_length * (cfg.terrain.num_rows - 1)

            low_y_ids = env_ids[self.root_states[self.ids_sim['robot'], 1] < thresh]
            self.root_states[low_y_ids, 1] += cfg.terrain.terrain_width * (cfg.terrain.num_cols - 1)

            high_y_ids = env_ids[
                self.root_states[self.ids_sim['robot'], 1] > cfg.terrain.terrain_width * cfg.terrain.num_cols - thresh]
    ...
    def _init_buffers(self):
        ...
        self.last_root_vel = torch.zeros_like(self.root_states[self.ids_sim['robot'], 7:13])
        ...
        if self.cfg.control.control_type == "actuator_net":
            actuator_path = f'{MINI_GYM_ROOT_DIR}/resources/actuator_nets/unitree_go1.pt'
    ...
    def _prepare_reward_function(self):
        ...
        """
        # reward containers
        from go1_gym_quart.envs.rewards.corl_rewards import CoRLRewards
    ...
    def __create_assets_handle(self, obj):
        # define object asset
        ...
        # load object asset
        # print(f'{MINI_GYM_ROOT_DIR}/assets/urdf/{obj}/', f'{obj}.urdf')
        object_asset = self.gym.load_asset(self.sim, f'{MINI_GYM_ROOT_DIR}/assets/urdf/{obj}/',
                                            f'{obj}.urdf', object_asset_options)
    ...
    def _create_envs(self):
        ...
        asset_path = self.cfg.asset.file.format(MINI_GYM_ROOT_DIR=MINI_GYM_ROOT_DIR) #go1.urdf?
        # asset_path = '/wangzhitao/walk-these-ways/resources/robots/wr2/wr2/wr2.urdf'
        ...
        base_pose.p = gymapi.Vec3(*self.base_init_state[:3])
        ...
        for i in range(self.num_envs):
            ...
            base_pose.p = gymapi.Vec3(*pos)
            if self.cfg.task== 'load':
                base_pose.p.z += 0.4   # unload任务时四足初始化位置
                # base_pose.p.z += 0.6
            else:
                base_pose.p.z += 0.5
            rigid_shape_props = self._process_rigid_shape_props(rigid_shape_props_asset, i)
            self.gym.set_asset_rigid_shape_properties(self.robot_asset, rigid_shape_props)
            anymal_handle = self.gym.create_actor(env_handle, self.robot_asset, base_pose, "anymal", i,
            ...
            if len(self.cfg.obj_name['target']) != 0:

                # print(self.cfg.obj_name['target'])

                if self.cfg.task != 'crawl':
                    base_pose.p.x += 3
                else:
                    base_pose.p.x += 2
                    base_pose.p.y += np.random.uniform(-0.8,0.8)
                base_pose.p.x += np.random.uniform(-0.3,0.3)


                all_obj_num = len(self.cfg.obj_name['target'])+int(len(self.cfg.obj_name['obstacle'])/self.num_envs)
                ypos = 1.5*(np.arange(all_obj_num)-np.mean(np.arange(all_obj_num)))
                np.random.shuffle(ypos)
                ## create target obj
                name,color,size,place = self.cfg.obj_name['target'][0],self.cfg.obj_color['target'][0],\
                                        self.cfg.obj_size['target'][0],self.cfg.obj_place['target'][0]
                if 'gate' in name and color!='red':
                    object_asset = self.__create_assets_handle(name+color)  #不同颜色的gate
                else:
                    object_asset = self.__create_assets_handle(name)

                start_pose = gymapi.Transform()
                start_pose.p = gymapi.Vec3(base_pose.p.x,base_pose.p.y,base_pose.p.z)

                # print('###############################################')
                if Cfg.task == 'crawl':
                    print('adjust the quat')
                    start_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0,0,1), np.radians(-90.0))

                if Cfg.task != 'crawl':
                    start_pose.p.y += ypos[len(self.ids_sim["object"])-i*all_obj_num]
                    start_pose.p.y += np.random.uniform(-0.1,0.1)
                    # start_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0,0,1), np.radians(0.0))

                if color == 'texture': 
                    start_pose.p.z = 0.
                    start_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0,0,1), np.radians(-90.0))

                object_handle = self.gym.create_actor(env_handle, object_asset, start_pose,
                                                    "object", i, self.cfg.asset.self_collisions, 0)

                if color != 'texture':

                    r,g,b = colors.hex2color(colors.cnames[color])
                    self.gym.set_rigid_body_color(env_handle, object_handle, 0,
                                                    gymapi.MESH_VISUAL_AND_COLLISION,  gymapi.Vec3(1,1,1))

                    tex_dim = 30
                    texture = np.ones((tex_dim,tex_dim,4))
                    texture[:,:,:3] = r,g,b
                    texture[::6,::6,:3] = 1-r,1-g,1-b
                    texture*=255.
                    texture = texture.reshape(tex_dim,tex_dim*4)
                    texture_handles = self.gym.create_texture_from_buffer(self.sim, tex_dim, tex_dim, texture.astype(np.uint8))
                    self.gym.set_rigid_body_texture(env_handle, object_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, texture_handles)
                object_idx = self.gym.get_actor_index(env_handle, object_handle, gymapi.DOMAIN_SIM)
                self.ids_sim["object"].append(object_idx)
                self.ids_sim["target"].append(object_idx)

                if len(self.cfg.obj_name['stop'][i]) != 0:

                    object_asset = self.__create_assets_handle('ball')
                    stop_pose = gymapi.Transform()
                    stop_pose.p = start_pose.p
                    stop_pose.p.x += 1

                    start_pose = gymapi.Transform()
                    start_pose.p = gymapi.Vec3(stop_pose.p.x,stop_pose.p.y,stop_pose.p.z)

                    stop_handle = self.gym.create_actor(env_handle, object_asset, start_pose,
                                                        "object", i, self.cfg.asset.self_collisions, 0)

                    r,g,b = colors.hex2color(colors.cnames['red'])

                    self.gym.set_rigid_body_color(env_handle, stop_handle, 0,
                    ...
                    texture = texture.reshape(tex_dim,tex_dim*4)
                    texture_handles = self.gym.create_texture_from_buffer(self.sim, tex_dim, tex_dim, texture.astype(np.uint8))
                    self.gym.set_rigid_body_texture(env_handle, stop_handle, 0, gymapi.MESH_VISUAL_AND_COLLISION, texture_handles)


            if len(self.cfg.obj_name['load'][i]) != 0:
                for name,color in zip(self.cfg.obj_name['load'][i],self.cfg.obj_color['load'][i]):
                    object_asset = self.__create_assets_handle(name)

                    pos = self.env_origins[i].clone() #这里与四足初始化位置中心保持一致
                    load_pose = gymapi.Transform()
                    load_pose.p = gymapi.Vec3(*pos)
                    # load_pose.p.z += 0.6  #小球初始化坐标
                    load_pose.p.z += 0.54  #小球初始化坐标
                    load_pose.p.x -= 0.05

                    start_pose = gymapi.Transform()
                    start_pose.p = gymapi.Vec3(load_pose.p.x,load_pose.p.y,load_pose.p.z)
                    if color == 'texture': 
                        start_pose.p.z = 0.
                        start_pose.r = gymapi.Quat.from_axis_angle(gymapi.Vec3(0,0,1), np.radians(-90.0))
                    ...
                    self.gym.set_actor_scale(env_handle, object_handle, 0.2)
                    ...
                    object_idx = self.gym.get_actor_index(env_handle, object_handle, gymapi.DOMAIN_SIM)
                    self.ids_sim["load"].append(object_idx)
                    # self.ids_sim["obstacle"].append(object_idx)

            ## create target obj
            if len(self.cfg.obj_name['obstacle'][i]) != 0:
                for name,color in zip(self.cfg.obj_name['obstacle'][i],self.cfg.obj_color['obstacle'][i]):
            ...
            if len(self.cfg.obj_name['avoid'][i]) != 0:
                for name,color in zip(self.cfg.obj_name['avoid'][i],self.cfg.obj_color['avoid'][i]):

                    object_asset = self.__create_assets_handle(name)

                    start_pose = gymapi.Transform()
                    start_pose.p = gymapi.Vec3(base_pose.p.x-1.5,base_pose.p.y,base_pose.p.z)
                    # start_pose.p.y += ypos[len(self.ids_sim["object"])-i*all_obj_num]
                    start_pose.p.y += np.random.uniform(-0.1,0.1)
                    if color == 'texture': 
                        start_pose.p.z = 0.
                    ...
                    # self.ids_sim["object"].append(object_idx)
                    self.ids_sim["obstacle"].append(object_idx)
                    # self.ids_sim["avoid"].append(object_idx)            

            # if len(self.cfg.obj_name['target']) != 0:
            #     base_pose.p.x += 3
            #     base_pose.p.x += np.random.uniform(-0.3,0.3)
            #     all_obj_num = len(self.cfg.obj_name['target'])+int(len(self.cfg.obj_name['obstacle'])/self.num_envs)
            #     ypos = 1.*(np.arange(all_obj_num)-np.mean(np.arange(all_obj_num)))
            #     np.random.shuffle(ypos)
            #     ## create target obj
            #     name,color,size,place = self.cfg.obj_name['target'][0],self.cfg.obj_color['target'][0],\
            #                             self.cfg.obj_size['target'][0],self.cfg.obj_place['target'][0]
            ...
            #     start_pose.p.y += ypos[len(self.ids_sim["object"])-i*all_obj_num]
            #     start_pose.p.y += np.random.uniform(-0.1,0.1)
            #     if color == 'texture': start_pose.p.z = 0.
            ...
            anymal_idx = self.gym.get_actor_index(env_handle, anymal_handle, gymapi.DOMAIN_SIM)
            self.ids_sim["robot"].append(anymal_idx)
            ...
            local_transform.p = gymapi.Vec3(*to_torch([0.3,0,0.0], device=self.device, requires_grad=False))
        ...
        for asset_name, asset_indices in self.ids_sim.items():
            self.ids_sim[asset_name] = torch.tensor(asset_indices, dtype=torch.long, device=self.device)
    ...
    def render(self):
        ...
        depths = []
        for i in range(self.num_envs):
            # bx, by, bz = self.root_states[i, 0], self.root_states[i, 1], self.root_states[i, 2]
            # self.gym.set_camera_location(self.camera_handles[i], self.envs[i], gymapi.Vec3(bx-1, by, bz + 1.0),
            ...
            depths.append(normalized_depth_image)
            # 

        return np.array(imgs), depths

    def _render_headless(self):
        bx, by, bz = self.root_states[0, 0], self.root_states[0, 1], self.root_states[0, 2]
        # pos = [bx + 0.0, by - 1.0, bz + 0.0]  # [m]
        pos = [bx - 5.0, by - 0.0, bz + 5.0]  # [m]
        # pos = [bx - 2.0, by - 0.0, bz + 2.0]  # [m]
        # pos = [bx - 2.0, by - 0.0, bz + 0.0]  # [m]
        lookat = [bx, by, bz]  # [m]
        self.set_camera(pos, lookat)
    ...
    def _get_env_origins(self, env_ids, cfg):
        ...
        if cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            if cfg.terrain.center_robots: #假设为9行9列，中心为1
                min_terrain_level = cfg.terrain.num_rows // 2 - cfg.terrain.center_span #3
                max_terrain_level = cfg.terrain.num_rows // 2 + cfg.terrain.center_span - 1 #4
                min_terrain_type = cfg.terrain.num_cols // 2 - cfg.terrain.center_span  #3
                max_terrain_type = cfg.terrain.num_cols // 2 + cfg.terrain.center_span - 1 #4
                self.terrain_levels[env_ids] = torch.randint(min_terrain_level, max_terrain_level + 1, (len(env_ids),),
                                                             device=self.device) #torch.randint(3,5,len) [3,5)范围生成[len,1]的值
            ...
            self.env_origins[env_ids] = cfg.terrain.terrain_origins[
                self.terrain_levels[env_ids], self.terrain_types[env_ids]] #随机从parameters.pkl的env_origins中选择一个坐标
        else:
