...
def get_sim_ranges(sample_rate, sim_instruction_dict, data_path, data_path_unload, info_path):
    ...
    command_name="commands_"+sample_rate+".npy"
    dict_path = os.path.join(info_path, command_name)
            ...
            if os.path.exists(episode_path): 
                ...
                if episode_path in all_dict.keys():  #'/dingpengxiang/Datasets/sim_quadruped_data_unload/unload_yellow_traybox/002288'
                    ...
                    for i in range(episode_length): 
                        ...
                        terminate = int(i == episode_length - 1)
                        # print([terminate, dx,dy,dyaw,body_height,step_frequency,gait_0,gait_1,gait_2,footswing_height,pitch,stance_width])
                        ...
                        vq_oneList.append([terminate, dx,dy,dyaw,body_height,step_frequency,gait_0,gait_1,gait_2,footswing_height,pitch,stance_width])
                        if terminate==1:
                            vq_DataList.append(vq_oneList)
                            vq_oneList=[]
                        # print(dict_json)

                        json_list.append(dict_json) 
                        image_id += 1  
    dict_path = os.path.join(info_path,'vq_data', f"vq_Dataset_{task}.npy")
    # print(vq_DataList)
    np.save(dict_path, vq_DataList)
    return image_id, json_list
...
if __name__ == "__main__":
    ...
    get_sim_ranges(sim_sample_rate, sim_instruction_dict, data_path, data_path_unload, info_path)
